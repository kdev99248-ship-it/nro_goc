    ...
    public static Player login(MySession session, AntiLogin al) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                long offlineDuration = System.currentTimeMillis() - lastTimeLogout;
                if (session.isAdmin) {
                    secondsPass = 100;
                    secondsPass1 = 100;
                }
...
//                    Service.gI().sendThongBaoOK(session, "Vui lòng đợi đến khi có thông báo mới nhất.");
//                }
            ...
            Logger.logException(SQLHandle.class, e);
        } finally {
            if (rs != null) {
                rs.dispose();
            }
    ...
    private static Player loadPlayer(NDVResultSet rs, boolean isOffline) throws Exception {
        ...
        try {
            ...
            System.err.println("name: " + player.name + " gem: " + player.inventory.gem);
...
//                    if (item.template.id == 595 && item.itemOptions.get(0).optionTemplate.id == 30) {
//                        item = ItemService.gI().createItemNull();
//                    }
                } else {
                    item = ItemService.gI().createItemNull();
            ...
            player.point_trung = Integer.parseInt(dataArray.get(0).toString());
            player.point_gioto = Integer.parseInt(dataArray.get(1).toString());
            player.point_banhday = Integer.parseInt(dataArray.get(2).toString());
            dataArray.clear();

            dataArray = (JSONArray) JSONValue.parse(rs.getString("drop_limit"));
            for (int i = 0; i < dataArray.size(); i++) {
                player.drop_limit[i] = Byte.parseByte(dataArray.get(i).toString());
            }
            ...
            try {
                ...
                player.gemChallenge = Long.parseLong(dataArray.get(2).toString());
