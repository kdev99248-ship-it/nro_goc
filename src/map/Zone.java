...
//    // ✅ THÊM: Update VirtualPlayer
//    try {
//        for (int i = this.getPlayers().size() - 1; i >= 0; i--) {
//            if (i < this.getPlayers().size()) {
//                Player pl = this.getPlayers().get(i);
//                if (pl != null && pl.zone != null && (pl instanceof VirtualPlayer || pl instanceof VirtualSoSinh)) {
//                    pl.update();
//                }
//            }
//        }
//    } catch (Exception e) {
//        Logger.logException(Zone.class, e, "Lỗi update VirtualPlayer");
//    }
}
...
public void load_Another_To_Me(Player player) { 
    try {
        if (MapService.gI().isMapOffline(this.map.mapId)) {
            ...
            // Load tất cả player, boss, pet VÀ BOT cho người chơi vừa vào
            for (int i = this.humanoids.size() - 1; i >= 0; i--) {
                Player pl = this.humanoids.get(i);
                if (pl != null && !player.equals(pl)) {
                    // ✅ Thêm điều kiện load cho VirtualPlayer và VirtualSoSinh
                    if (pl instanceof VirtualPlayer || pl instanceof VirtualSoSinh) {
                        infoPlayer(player, pl);
                    } else if (!(pl instanceof VirtualPlayer) && !(pl instanceof VirtualSoSinh)) {
                        infoPlayer(player, pl);
                    }
                }
            }
...
// Và cũng sửa load_Me_To_Another để bot hiển thị cho người khác
    ...
    try {
        if (player.zone != null) {
            if (MapService.gI().isMapOffline(this.map.mapId)) {
                ...
                // ✅ Load cho tất cả player kể cả bot
                for (int i = players.size() - 1; i >= 0; i--) {
                    Player pl = players.get(i);
                    if (!player.equals(pl)) {
                        // Gửi thông tin player/bot mới cho những người đang ở trong map
                        infoPlayer(pl, player);
                    }
                }
            }
    ...
    public void infoPlayer(Player plReceive, Player plInfo) {
        ...
        try {
            ...
            if (plInfo.nPoint != null) {
                ...
                System.err.println("nPoint null tại infoPlayer cho player id: " + plInfo.id);
            }
            ...
            if (plReceive.inventory != null && plReceive != null && plReceive.isPl() && plReceive.getSession() != null && plReceive.getSession().version >= 228) {
                if (flagbag == 83) {
                    flagbag = 205;
                }
            }
...
//                }
            ...
            e.printStackTrace(); // Log lỗi khi gửi pet/chibi
        }
        ...
        try {
            ...
            e.printStackTrace(); // Log lỗi nếu có trong phần báo chết
        }
    }
...
//    //Code them vao
//    public Player getNearestPlayer(Boss boss) {
//    Player nearest = null;
//    int minDistance = Integer.MAX_VALUE;
//    for (Player pl : this.players) {
//        if (pl != null && !pl.isDie()) {
//            int dist = Math.abs(pl.location.x - boss.location.x);
//            if (dist < minDistance) {
//                minDistance = dist;
//                nearest = pl;
//            }
