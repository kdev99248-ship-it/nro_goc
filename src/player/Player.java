 ...
 * @author HM
 */
import DucVuPro.Functions;
import bot.VirtualPlayer;
import bot.VirtualSoSinh;
import clan.Clan;
...
public class Player implements Runnable {
    ...
    //TASK FIELD
    public byte PK_THAN_MEO = 0;
    public byte[] drop_limit;
    ...
    public long lastTimeAutoRecover = 0;
...
//                    }
                    ...
                    if (this.isPl() && !this.isDie() && this.effectSkill != null && !this.effectSkill.isChibi
                        ...
                        if (Util.isTrue(1, 200) && !MapService.gI().isMapBlackBallWar(this.zone.map.mapId)) {
...
//                    }
                    if (this.isPl()) {
                        ...
                        if (!(hour >= 22 && hour <= 23)) {
                            if (this.zone != null && zone.map.mapId == 126) {
...
//                        sendTextTimeDaiLyGift();
                    ...
                    if (this.isPl() && this.nPoint.power > 200_000_000L && this.superRank != null) {
    ...
    public boolean canBuyBackCaiTrang() {
        if (inventory != null && inventory.itemsBody != null && inventory.itemsBody.get(5) != null) {
            for (ItemOption option : inventory.itemsBody.get(5).itemOptions) {
                if (option != null && option.optionTemplate.id != 252 && option.optionTemplate.id != 93 && option.optionTemplate.id != 154) {
                    return true;
                }
            }
        }
    ...
    public Item getItemBuyBack() {
        return inventory.itemsBody.get(5);
    }
...
//        }};
    ...
    public byte getAura() {
        ...
        for (Card card : this.Cards) {
            if (card != null && (card.Id == 956 || card.Id == 1204 || card.Id == 1771) && card.Level > 1) {
    ...
    public short getHead() {
        if(this instanceof VirtualPlayer || this instanceof VirtualSoSinh){
            return head;
        }
        boolean flag = this.isPl();
...
//        }
    ...
    public short getMount() {
        ...
        if (item.template.type == 24) {
            ...
            if (item.template.id < 500) {
                ...
                return (short) DataGame.MAP_MOUNT_NUM.getOrDefault(item.template.id, (short) -1);
            }
        }
    }
    ...
    public synchronized int injured(Player plAtt, long damage, boolean piercing, boolean isMobAttack) {
        if (!this.isDie()) {
                ...
                switch (plAtt.playerSkill.skillSelect.template.id) {
                    case Skill.KAMEJOKO, Skill.MASENKO, Skill.ANTOMIC -> {
                        if (this.nPoint.voHieuChuong > 0) {
                            SkillService.gI().phanSatThuong(plAtt, this, damage);
            ...
            if (plAtt != null && (plAtt.isPl() || plAtt.isBoss)) {
                this.markAttackedByPlOrBoss();
            }
            if (this.zone.map.mapId == 51) {
                this.totalDamageTaken += damage;
            }
            damage = Math.min(damage, this.nPoint.hp);
            if (damage < 0) {
                damage = 0;
            }
    ...
    protected void setDie(Player plAtt) {
        ...
        if (this.effectSkill.isChibi) {
            EffectSkillService.gI().removeChibi(plAtt);
        }
    ...
    // }
    // }
     public void setInfo(int hp, int mp, int dame, int def, int crit) {
        nPoint.hpg = hp;
        nPoint.mpg = mp;
        nPoint.dameg = dame;
        nPoint.defg = def;
        nPoint.critg = crit;
          nPoint.stamina =  nPoint.maxStamina = 32000;
        Service.gI().point(this);
    }
     public void setPos(int xPos, int yPos){
         location.x = xPos;
         location.y = yPos;
         Service.gI().setPos(this, xPos, yPos);
     }
     public int getX(){
         return location.x;
     }
    ...
    //TuanBinh
    public long lastTimeAttackedByPlOrBoss = 0;

    public void markAttackedByPlOrBoss() {
        this.lastTimeAttackedByPlOrBoss = System.currentTimeMillis();
    }

    public boolean isAttackedByPlayerOrBoss(long durationMs) {
        return System.currentTimeMillis() - this.lastTimeAttackedByPlOrBoss <= durationMs;
    }
